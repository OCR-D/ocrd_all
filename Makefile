...
clean: # add more prerequisites for clean below
	...
	$(RM) ocrd-all-tool.json ocrd-all-module-dir.json ocrd-all-dockerhub.json
...
Targets (auxiliary data):
	...
	ocrd-all-dockerhub.json: mapping of Docker image name for all executables of all modules
...
all: ocrd-all-tool.json ocrd-all-module-dir.json ocrd-all-dockerhub.json
...
ocrd-all-dockerhub.json: $(OCRD_MODULES) $(ACTIVATE_VENV)
	. $(ACTIVATE_VENV) && $(PYTHON) ocrd-all-dockerhub.py $(wildcard $(OCRD_MODULES:%=%/ocrd-tool.json)) > $@
